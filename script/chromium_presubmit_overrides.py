...
# Override canned_checks to ignore/modify upstream validators.
def _override_canned_checks(input_api):
...
def OverriddenFirstCheck(original_check, input_api, output_api, *args,
                         **kwargs):
    _override_canned_checks(input_api)
...
@chromium_presubmit_utils.override_check(globals())
def CheckNoPragmaOnce(original_check, input_api, output_api, *args, **kwargs):
...
def CheckMPArchApiUsage(original_check, input_api, output_api, *args,
    ...
    def AffectedFiles(self, original_method, *_, **__):
        source_file_filter = lambda f: self.FilterSourceFile(f)
        return original_method(False, source_file_filter)

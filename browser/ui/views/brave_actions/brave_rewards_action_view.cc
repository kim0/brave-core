...
gfx::Rect BraveRewardsActionView::GetAnchorBoundsInScreen() const {
  if (!GetVisible()) {
    ...
    // TODO(zenparsing): Should we anchor it to the action view instead?
...
void BraveRewardsActionView::OnButtonPressed() {
  ...
  // Even though the bubble is hidden, the Rewards panel may be running in the
  // background for cached access and listening to the panel service. In order
  // to notify the panel, request that the panel be opened through the panel
  // service, rather than opening the bubble directly.
  if (panel_service_) {
...
bool BraveRewardsActionView::UpdatePublisherStatus() {
  ...
  // TODO(zenparsing): When rewards is enabled, should we automatically check
  // this again? We can use |OnRewardsInitialized|, but it doesn't really work
  // because the publisher index has not been fully populated yet. We may need
  // to introduce an event that is triggered when the publisher index has been
  // updated.
  // TODO(zenparsing): We also need to update the badge when the user manually
  // refreshes the publisher status from the Rewards panel UI. It looks like
  // we'll need another event for that.

...
void RewardsServiceImpl::StartLedgerProcessIfNecessary() {
  ...
  HandleFlags(RewardsArgs::ForCurrentProcess());
...
// static
void RewardsServiceImpl::HandleFlags(const RewardsArgs& args) {
  if (args.environment) {
    switch (*args.environment) {
      case RewardsArgs::Environment::kDevelopment:
        SetEnvironment(ledger::type::Environment::DEVELOPMENT);
        break;
      case RewardsArgs::Environment::kStaging:
        SetEnvironment(ledger::type::Environment::STAGING);
        break;
      case RewardsArgs::Environment::kProduction:
  ...
  if (args.reconcile_interval) {
    SetReconcileInterval(*args.reconcile_interval);
  }

  if (args.retry_interval) {
    SetRetryInterval(*args.retry_interval);
  }

  if (args.gemini_retries) {
    SetGeminiRetries(*args.gemini_retries);
  }
  ...
  // instead.
  if (args.persist_logs) {
    persist_log_level_ = kDiagnosticLogMaxVerboseLevel;
  }

  if (args.country_id) {
    country_id_ = *args.country_id;
  }
}

void RewardsServiceImpl::HandleFlagsForTesting(const std::string& input) {
  return HandleFlags(RewardsArgs::Parse(input));
}

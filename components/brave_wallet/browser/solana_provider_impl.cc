...
absl::optional<SolanaMessage> SolanaProviderImpl::GetDeserializedMessage(
  ...
  // 1. Serialize the created message object and encode in Base58 will be the
  //    same as original encoded message.
  // 2. Fee payer should be the current selected account.
  ...
  if (!serialized_message ||
      ...
      account != msg->fee_payer() || signers.size() != 1) {
    return absl::nullopt;
  }
